...
function MissionFilter ({ launches, launchpads }) {
    ...
    const handleFormSubmit = (event) => {
        ...
        if (minYear) {
            filterData = filterData.filter(item => (
                new Date(minYear).getFullYear() <= new Date(item.launch_date_local).getFullYear()
            ));
        }

        if (maxYear) {
            filterData = filterData.filter(item => (
                new Date(maxYear).getFullYear() >= new Date(item.launch_date_local).getFullYear()
            ));
        }
